...
class Robby():
  def __init__(self):    
    self.QMatrix = np.zeros(((3*3*3*3*3),(len(self.actions))))
    self.Learn = True
  ...
  def getState(self):
    # Returns a list [north, east, south, west, current]
    # 1 if can, -1 if wall, 0 if empty
    state = [self.northStatus(), self.eastStatus(), self.southStatus(), self.westStatus(), self.currentStatus()]
    return state
